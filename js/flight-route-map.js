/**
 *  Drawing animated plane from destination 1 to destination 2 over google maps
 *  information data is retrived from ajax request as objects with following information:
 *  {
 *    dest1: {
 *      title: 'tile of destination 1',     
 *      lat: 0.000454, // latitude of destination 1
 *      lng: 0.000454, // longitude of destination 1   
 *    }
 *    dest2: {
 *      title: 'tile of destination 2',     
 *      lat: 0.000454, // latitude of destination 2
 *      lng: 0.000454, // longitude of destination 2   
 *    }
 *  }
 *
 *
 *  USAGE:
 *  <div id="example" data-dest1="KIV" data-dest2="PAR"></div>
 *  $('#example').flightRouteMap();
 */

(function($) {
    $.fn.flightRouteMap = function(options) {        
        if(google.maps == undefined){
            console.log('flightRouteMap: Google maps is not defined/loaded. Load google script maps first!');
            return false;
        }
        
        var settings = $.extend({
            mapWidth: 880,
            mapHeight: 270,
            colors: {
                water: '#00b0dd',
                landscape: '#c1e2f4',
                plane: '#5283c3',
                path: '#ffffff'
            },
            marker: {
                image: 'images/dot.png',
                width: 24,
                height: 24,
                labelClass: 'markerLabel'
            },
            requestRoute: 'routes/ajax',
        }, options)

        var self = this;

        self.each(function(){
            var elem = $(this);
            elem.width(settings.mapWidth); 
            elem.height(settings.mapHeight);
            if((elem.data('dest1') != undefined) && (elem.data('dest2') != undefined)){
                $.post(
                    settings.requestRoute, 
                    data:{
                        dest1:elem.data('dest1'),
                        dest2:elem.data('dest2')
                    }
                )
                .done(function(response){ 
                    if((typeof(response) == 'object') && count(response) == 2){
                        createMap(elem,response.dest1,response.dest2);
                    } else {
                        console.log('flightRouteMap: Data response must be an object!');
                    }
                }).fail(function(response){
                    console.log('flightRouteMap: Request fail!');
                });
            } else {
                console.log('flightRouteMap: Invalid destinations data atributes!');
            }
        });        
        
        function setMapMarker(map,position,title,content){
            var marker,infoWindow;
            var markerOptions = {position:position,map:map};
            var mapMarkerImage = new google.maps.MarkerImage(
                settings.marker.image,
                new google.maps.Size(settings.marker.width,settings.marker.height),
                new google.maps.Point(0,0),
                new google.maps.Point(parseInt(settings.marker.width/2),parseInt(settings.marker.height/2))
            );
            markerOptions.icon = mapMarkerImage;
            if(title != undefined){
                markerOptions.title = title;
                markerOptions.labelContent = title;
                markerOptions.labelAnchor = new google.maps.Point(20, -15);
                markerOptions.labelClass = settings.marker.labelClass;
                markerOptions.labelStyle = {opacity:1};
            }
            marker = new MarkerWithLabel(markerOptions);            
            if(content != undefined){
                google.maps.event.addListener(marker, 'click', function(){
                    if (infoWindow !== void 0){infoWindow.close();}
                    var infoWindowOptions = {content: content};
                    infoWindow = new google.maps.InfoWindow(infoWindowOptions);
                    infoWindow.open(map,marker);
                });
            }
            return marker;
        }
        
        function createMap(target,dest1,dest2) {
            var map, markers=[], bounds;
            var startPoint = new google.maps.LatLng(dest1.lat,dest1.lng);
            var endPoint = new google.maps.LatLng(dest2.lat,dest2.lng);
            var customMapType = new google.maps.StyledMapType(
                [
                    {featureType:'all',stylers:[{visibility: 'off'}]},
                    {featureType:'landscape',elementType:'geometry.fill',stylers:[{visibility:'on'},{color:settings.colors.landscape}]},
                    {featureType:'administrative.country',elementType:'geometry.stroke',stylers:[{visibility:'on'},{color:settings.colors.water},{weight:.7},{lightness:-15}]},
                    {featureType:'water',stylers:[{visibility:'on'},{color:settings.colors.water}]}
                ],
                {name:'zbor.md'}
            );
            var customMapTypeId = 'zbormd_style';
            var mapOptions = {center:startPoint,zoom:2,mapTypeId:google.maps.MapTypeId.ROADMAP,scrollwheel:false,mapTypeControlOptions:{mapTypeIds:[google.maps.MapTypeId.ROADMAP,customMapTypeId]}};
            map = new google.maps.Map(target, mapOptions);
            map.mapTypes.set(customMapTypeId, customMapType);
            map.setMapTypeId(customMapTypeId);
            bounds = new google.maps.LatLngBounds();
            var planeImg = {
                path:'m15.51546,-1.53623c-0.01863,-0.02964 -0.91712,-1.78028 -1.99666,-3.89033c-1.07953,-2.11005 -1.97956,-3.85376 -2.00007,-3.87491c-0.02394,-0.0247 -0.59527,0.52725 -1.59643,1.5423c-1.46259,1.48288 -1.55911,1.58936 -1.55855,1.71939c0.00033,0.07624 0.06044,0.63696 0.13358,1.24604l0.13298,1.10743l-0.63202,0.64375l-0.63202,0.64375l-0.73344,-1.41589l-0.73344,-1.4159l-1.38031,-0.7308c-0.75917,-0.40194 -1.3807,-0.73991 -1.38117,-0.75104c-0.00047,-0.01113 0.27827,-0.30289 0.61943,-0.64836l0.62029,-0.62811l1.21672,0.14976l1.21672,0.14976l1.57597,-1.59761c1.22897,-1.24585 1.56397,-1.60632 1.52148,-1.63716c-0.02997,-0.02175 -1.78521,-0.96497 -3.90054,-2.09603l-3.84605,-2.05648l0.79426,-0.80547l0.79427,-0.80547l4.74741,1.27587l4.74741,1.27587l2.17078,-2.19703c1.52155,-1.53994 2.22328,-2.22372 2.34629,-2.28629c0.11329,-0.05762 0.27283,-0.09571 0.45008,-0.10745c0.34871,-0.0231 0.60821,0.08026 0.83903,0.3342c0.21449,0.23597 0.284,0.41398 0.28304,0.72482c-0.00147,0.48012 0.0253,0.44764 -2.41243,2.92677l-2.19731,2.23463l0.94913,3.71779c0.52202,2.04478 1.06289,4.16512 1.20192,4.71186l0.25279,0.99407l-0.78963,0.80006c-0.5867,0.59445 -0.79834,0.78621 -0.8235,0.74617z',
                scale:1,strokeColor:'#5283c3',fillColor:'#5283c3',fillOpacity:1,rotation:315
            };
            var flightPathOptions = {path:[startPoint,endPoint],geodesic:true,radius:1500,strokeColor:settings.colors.path,strokeOpacity:1.0,strokeWeight:3,icons:[{icon:planeImg,offset:'0%'}],map:map};
            var flightPath = new google.maps.Polyline(flightPathOptions);
            var marker1 = setMapMarker(map,startPoint,dest1.tite);
            markers.push(marker1);
            bounds.extend(marker1.position);
            var marker2 = setMapMarker(map,endPoint,dest2.title);
            markers.push(marker2);            
            bounds.extend(marker.position);
            map.fitBounds(bounds);
            animatePlane(flightPath);
        }
        
        function animatePlane(polyLine){            
            var count = 0;
            offsetId = window.setInterval(function() {
                var icons = polyLine.get('icons');
                count = (count + 0.25); 
                icons[0].offset = count + '%';
                polyLine.set('icons', icons);
                if(count == 90) count = 0;
            }, 20);
        }
    }
})(jQuery)